...
public class Reader {
    ...
    public OpenAPI read(Set<Class<?>> classes) {
        ...
        for (Class<?> cls : sortedClasses) {
            if (
                ReaderListener.class.isAssignableFrom(cls) &&
                !listeners.containsKey(cls)
            ) {
                try {
    ...
    protected String resolveApplicationPath() {
        if (application != null) {
            ...
            while (
                (applicationPath = applicationToScan.getAnnotation(
            ...
            try {
                ...
                Method m = application
                    .getClass()
                    .getMethod("getApplication", null);
                while (m != null) {
                    Application retrievedApp = (Application) m.invoke(
                        innerApp,
                        null
                    );
    ...
    public OpenAPI read(
        ...
        // class path
        final jakarta.ws.rs.Path apiPath = ReflectionUtils.getAnnotation(
            cls,
        ...
        > apiSecurityScheme = ReflectionUtils.getRepeatableAnnotations(
            cls,
            io.swagger.v3.oas.annotations.security.SecurityScheme.class
        );
        List<
            io.swagger.v3.oas.annotations.security.SecurityRequirement
        > apiSecurityRequirements = ReflectionUtils.getRepeatableAnnotations(
            cls,
            io.swagger.v3.oas.annotations.security.SecurityRequirement.class
        );

        ExternalDocumentation apiExternalDocs = ReflectionUtils.getAnnotation(
            cls,
        ...
        Server[] apiServers = ReflectionUtils.getRepeatableAnnotationsArray(
            cls,
            Server.class
        );

        Consumes classConsumes = ReflectionUtils.getAnnotation(
            cls,
            Consumes.class
        );
        Produces classProduces = ReflectionUtils.getAnnotation(
            cls,
            Produces.class
        );
        ...
        if (openAPIDefinition != null) {
            ...
            SecurityParser.getSecurityRequirements(
                openAPIDefinition.security()
            ).ifPresent(s -> openAPI.setSecurity(s));
            //
            // OpenApiDefinition external docs
            AnnotationsUtils.getExternalDocumentation(
                openAPIDefinition.externalDocs()
            ).ifPresent(docs -> openAPI.setExternalDocs(docs));

            // OpenApiDefinition tags
            AnnotationsUtils.getTags(openAPIDefinition.tags(), false).ifPresent(
                tags -> openApiTags.addAll(tags)
            );

            // OpenApiDefinition servers
            AnnotationsUtils.getServers(openAPIDefinition.servers()).ifPresent(
                servers -> openAPI.setServers(servers)
            );

            // OpenApiDefinition extensions
            if (openAPIDefinition.extensions().length > 0) {
                openAPI.setExtensions(
                    AnnotationsUtils.getExtensions(
                        openAPIDefinition.extensions()
                    )
                );
            }
        ...
        if (apiSecurityScheme != null) {
            for (io.swagger.v3.oas.annotations.security.SecurityScheme securitySchemeAnnotation : apiSecurityScheme) {
                Optional<SecurityParser.SecuritySchemePair> securityScheme =
                ...
                if (securityScheme.isPresent()) {
                    ...
                    if (StringUtils.isNotBlank(securityScheme.get().key)) {
                        securitySchemeMap.put(
                            securityScheme.get().key,
                            securityScheme.get().securityScheme
                        );
                        if (
                            components.getSecuritySchemes() != null &&
                            components.getSecuritySchemes().size() != 0
                        ) {
                            components
                                .getSecuritySchemes()
                                .putAll(securitySchemeMap);
                        } else {
            ...
            Optional<List<SecurityRequirement>> requirementsObject =
                SecurityParser.getSecurityRequirements(
                    apiSecurityRequirements.toArray(
                        new io.swagger.v3.oas.annotations.security.SecurityRequirement[apiSecurityRequirements.size()]
                    )
                );
        ...
        for (Method method : methods) {
            ...
            Class<?>[] parameterTypes = Arrays.stream(
            ...
            Consumes methodConsumes = ReflectionUtils.getAnnotation(
                method,
                Consumes.class
            );
            ...
            jakarta.ws.rs.Path methodPath = ReflectionUtils.getAnnotation(
                method,
                jakarta.ws.rs.Path.class
            );

            String operationPath = ReaderUtils.getPath(
                apiPath,
                methodPath,
                parentPath,
                isSubresource
            ...
            if (
                ignoreOperationPath(operationPath, parentPath) && !isSubresource
            ) {
                continue;
            }
            ...
            if (operationPath != null) {
                if (
                    config != null &&
                    ReaderUtils.isIgnored(operationPath, config)
                ) {
                    continue;
                }
                ...
                String httpMethod = ReaderUtils.extractOperationMethod(
                    method,
                    OpenAPIExtensions.chain()
                );
                httpMethod = (httpMethod == null && isSubresource)
                    ? parentMethod
                    : httpMethod;
                ...
                if (StringUtils.isBlank(httpMethod) && subResource == null) {
                    ...
                    StringUtils.isBlank(httpMethod) && subResource != null
                ) {
                ...
                JsonView jsonViewAnnotation = ReflectionUtils.getAnnotation(
                    method,
                    JsonView.class
                );
                ...
                if (operation != null) {
                    ...
                    Annotation[][] paramAnnotations = getParameterAnnotations(
                        method
                    );
                    if (annotatedMethod == null) {
                        ...
                        genericParameterTypes = Arrays.stream(
                            genericParameterTypes
                        )
                            .filter(t ->
                                !t.getTypeName().contains("ServerRequest")
                            )
                            .toArray(Type[]::new);
                        //
                        ...
                        for (int i = 0; i < genericParameterTypes.length; i++) {
                            ...
                            Type paramType =
                                ParameterProcessor.getParameterType(
                                    paramAnnotation,
                                    true
                                );
                            ...
                            operationParameters.addAll(
                                resolvedParameter.parameters
                            );

                            resolvedParameter.formParameters
                                .stream()
                                .forEach(fp -> fp.setRequired(!isOptional));

                            formParameters.addAll(
                                resolvedParameter.formParameters
                            );
                        ...
                        for (
                            int i = 0;
                            i < annotatedMethod.getParameterCount();
                            i++
                        ) {
                    ...
                    if (formParameters.size() > 0) {
                        ...
                        for (Parameter formParam : formParameters) {
                            if (
                                formParam.getRequired() != null &&
                                formParam.getRequired()
                            ) {
                                isRequired = true;
                            }

                            mergedSchema.addProperties(
                                formParam.getName(),
                                formParam.getSchema()
                            );
                        }
                    ...
                    if (method.isAnnotationPresent(Chain.class)) {
                        Chain chainAnnotation = method.getAnnotation(
                            Chain.class
                        );

                        Class<? extends HandlerWrapper>[] wrappers =
                            chainAnnotation.value();
                        ...
                        > wrapper : wrappers) {
                            if (wrapper.equals(JsonViewWrapper.class)) {
                                LOGGER.debug(
                                    "Found json wrapper class on method"
                                );
                                hasJsonWrapper = true;
                            }
                        }
                        ...
                        > wrapper : wrappers) {
                            if (wrapper.equals(JsonViewWrapper.class)) {
                                LOGGER.debug(
                                    "Found json wrapper class on class"
                                );
                                hasJsonWrapper = true;
                            }
                        }
                    ...
                    if (
                        ...
                        config
                            ...
                            .containsKey("jsonViewQueryParameterName")
                    ) {
                        ...
                        contextParameter
                            ...
                            .name(
                                config
                                    .getUserDefinedOptions()
                                    .get("jsonViewQueryParameterName")
                                    .toString()
                            )
                            .in("query")
                            .schema(stringSchema);
                    ...
                    if (
                        subResource != null &&
                        !scannedResources.contains(subResource)
                    ) {
                    ...
                    if (
                        ...
                        openAPI.getPaths().get(operationPath) != null
                    ) {
    ...
    public static Annotation[][] getParameterAnnotations(Method method) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (
                !params[i].getType().isAssignableFrom(ServerRequest.class) &&
                !params[i].getType().getName().startsWith("io.undertow")
            ) {
        ...
        ArrayList<Annotation[]> annotations = Arrays.stream(
            methodAnnotations
        ...
        if (overriddenmethod != null) {
            Annotation[][] overriddenAnnotations =
                overriddenmethod.getParameterAnnotations();
            ...
            for (int i = 0; i < methodAnnotations.length; i++) {
                ...
                for (int j = 0; j < overriddenAnnotations[i].length; j++) {
                    if (
                        !types.contains(
                            overriddenAnnotations[i][j].annotationType()
                        )
                    ) {
    ...
    protected void processRequestBody(
        ...
        boolean isOptional = !(requestBodyParameter.getRequired() != null
            ? requestBodyParameter.getRequired()
            : true);
        ...
        io.swagger.v3.oas.annotations.parameters.RequestBody requestBodyAnnotation =
            getRequestBody(Arrays.asList(paramAnnotations));
        if (requestBodyAnnotation != null) {
            Optional<RequestBody> optionalRequestBody =
                OperationParser.getRequestBody(
            ...
            if (optionalRequestBody.isPresent()) {
                ...
                if (
                    StringUtils.isBlank(requestBody.get$ref()) &&
                    ...
                    if (requestBodyParameter.getSchema() != null) {
                        Content content = processContent(
                            ...
                            requestBodyParameter.getSchema(),
                            methodConsumes,
                            classConsumes
                        );
                    ...
                    if (requestBodyParameter.getSchema() != null) {
                        for (MediaType mediaType : requestBody
                            .getContent()
                            .values()) {
            ...
            if (operation.getRequestBody() == null) {
                ...
                if (
                    StringUtils.isNotBlank(
                        requestBodyParameter.getDescription()
                    )
                ) {
    ...
    private io.swagger.v3.oas.annotations.parameters.RequestBody getRequestBody(
    ...
    private void setMediaTypeToContent(
        Schema schema,
        Content content,
        String value
    ) {
    ...
    private Operation parseMethod(
        ...
        if (
            Arrays.stream(method.getParameters())
                .filter(p ->
                    p.getType().isAssignableFrom(HttpServerExchange.class)
                )
                .count() >
            0L
        ) {
        ...
        List<
            io.swagger.v3.oas.annotations.security.SecurityRequirement
        > apiSecurity = ReflectionUtils.getRepeatableAnnotations(
            method,
            io.swagger.v3.oas.annotations.security.SecurityRequirement.class
        );
        ...
        List<Server> apiServers = ReflectionUtils.getRepeatableAnnotations(
            method,
            Server.class
        );
        ...
        if (apiCallbacks != null) {
            for (io.swagger.v3.oas.annotations.callbacks.Callback methodCallback : apiCallbacks) {
                Map<String, Callback> currentCallbacks = getCallbacks(
                    methodCallback,
                    methodProduces,
                    classProduces,
                    methodConsumes,
        ...
        if (apiSecurity != null) {
            Optional<List<SecurityRequirement>> requirementsObject =
                SecurityParser.getSecurityRequirements(
                    apiSecurity.toArray(
                        new io.swagger.v3.oas.annotations.security.SecurityRequirement[apiSecurity.size()]
                    )
                );
            if (requirementsObject.isPresent()) {
                requirementsObject
        ...
        if (apiServers != null) {
            AnnotationsUtils.getServers(
                apiServers.toArray(new Server[apiServers.size()])
            ...
            apiTags
                ...
                .filter(
                    t ->
                        ...
                        (operation.getTags() != null &&
                            !operation.getTags().contains(t.name()))
                )
                .map(t -> t.name())
                .forEach(operation::addTagsItem);
            AnnotationsUtils.getTags(
                apiTags.toArray(
                    new io.swagger.v3.oas.annotations.tags.Tag[apiTags.size()]
                ),
                true
        ...
        if (apiParameters != null) {
            getParametersListFromAnnotation(
                apiParameters.toArray(
                    new io.swagger.v3.oas.annotations.Parameter[apiParameters.size()]
                ),
                classConsumes,
                methodConsumes,
                operation,
        ...
        if (apiRequestBody != null && operation.getRequestBody() == null) {
            ...
            LOGGER.debug(
                "request body: " + operation.getRequestBody().toString()
            );
        }
        ...
        if (apiResponses != null && apiResponses.size() > 0) {
            OperationParser.getApiResponses(
                apiResponses.toArray(
                    new io.swagger.v3.oas.annotations.responses.ApiResponse[apiResponses.size()]
                ),
                classProduces,
                methodProduces,
                components,
        ...
        if (isSubresource && parentRequestBody != null) {
            if (operation.getRequestBody() == null) {
                ...
                if (content == null) {
                    ...
                    for (String parentMediaType : parentRequestBody
                        .getContent()
                        .keySet()) {
    ...
    private Map<String, Callback> getCallbacks(
        ...
        for (io.swagger.v3.oas.annotations.Operation callbackOperation : apiCallback.operation()) {
            ...
            setPathItemOperation(
                pathItemObject,
                callbackOperation.method(),
                callbackNewOperation
            );
        }

        callbackObject.addPathItem(
            apiCallback.callbackUrlExpression(),
            pathItemObject
        );
        callbackMap.put(apiCallback.name(), callbackObject);
    ...
    private void setPathItemOperation(
        PathItem pathItemObject,
        String method,
        Operation operation
    ) {
    ...
    protected void setOperationObjectFromApiOperationAnnotation(
        ...
        if (StringUtils.isNotBlank(apiOperation.operationId())) {
            operation.setOperationId(
                getOperationId(apiOperation.operationId())
            );
        }
        ...
        ReaderUtils.getStringListFromStringArray(apiOperation.tags()).ifPresent(
            tags -> {
                tags
                    .stream()
                    .filter(
        ...
        if (
            ...
            OperationParser.getRequestBody(
                ...
                components,
                jsonViewAnnotation
            ).ifPresent(requestBodyObject ->
                operation.setRequestBody(requestBodyObject)
            );
        }

        // Extensions in Operation
        if (apiOperation.extensions().length > 0) {
            Map<String, Object> extensions = AnnotationsUtils.getExtensions(
    ...
    protected ResolvedParameter getParameters(
        ...
        final ResolvedParameter extractParametersResult =
            extension.extractParameters(
                annotations,
                type,
                typesToSkip,
                components,
    ...
    private boolean isEmptyComponents(Components components) {
        ...
        if (
            components.getCallbacks() != null &&
            components.getCallbacks().size() > 0
        ) {
            return false;
        }
        if (
            components.getExamples() != null &&
            components.getExamples().size() > 0
        ) {
            return false;
        }
        if (
            components.getExtensions() != null &&
            components.getExtensions().size() > 0
        ) {
            return false;
        }
        if (
            components.getHeaders() != null &&
            components.getHeaders().size() > 0
        ) {
            return false;
        }
        ...
        if (
            ...
            components.getParameters().size() > 0
        ) {
            return false;
        }
        if (
            components.getRequestBodies() != null &&
            components.getRequestBodies().size() > 0
        ) {
            return false;
        }
        if (
            components.getResponses() != null &&
            components.getResponses().size() > 0
        ) {
            return false;
        }
    ...
    protected boolean isOperationHidden(Method method) {
        ...
        if (
            config != null &&
            !Boolean.TRUE.equals(config.isReadAllResources()) &&
            apiOperation == null
        ) {
            return true;
        }
    ...
    protected boolean ignoreOperationPath(String path, String parentPath) {
        if (StringUtils.isBlank(path) && StringUtils.isBlank(parentPath)) {
            ...
            StringUtils.isNotBlank(path) && StringUtils.isBlank(parentPath)
        ) {
            return false;
        } else if (
            StringUtils.isBlank(path) && StringUtils.isNotBlank(parentPath)
        ) {
            return false;
        }
        if (
